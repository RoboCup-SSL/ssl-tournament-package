...
- Package installs / module fetches resolve through the pre-configured package sources (`package-sources.internal`). Never add `--registry`/`--index-url`/`GOPROXY` overrides; if a fetch fails, STOP and report.
...
The environment resolves npm through the pre-configured package sources; use plain npm with no registry flags. If the fetch fails, STOP and report — do not point npm anywhere else.
